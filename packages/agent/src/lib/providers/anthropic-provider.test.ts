...
import type { Listing, SoldListing, AnalyzePrompt } from "@synthfinder/shared";
...
function prompt(overrides: Partial<AnalyzePrompt> = {}): AnalyzePrompt {
  return {
    listings: [fixtureListing, fixtureListing2],
    soldListings: fixtureSoldListings,
    ...overrides,
  };
}
...
describe("AnthropicProvider", () => {
  ...
  it("implements LLMProvider and makes a single API call for all listings", async () => {
    ...
    const results = await new AnthropicProvider().analyzeListings(prompt());
  ...
  it("includes all listing titles and sold comps in the prompt", async () => {
    ...
    const { AnthropicProvider } = await import("./anthropic-provider.js");
    await new AnthropicProvider().analyzeListings(prompt({ listings: [fixtureListing] }));
  ...
  it("maps results to ScoredListing[] preserving originalListing", async () => {
    ...
    const { AnthropicProvider } = await import("./anthropic-provider.js");
    const results = await new AnthropicProvider().analyzeListings(
      prompt({ listings: [fixtureListing] }),
    );
  ...
  it("throws when the API response contains no tool_use block", async () => {
    ...
    const { AnthropicProvider } = await import("./anthropic-provider.js");
    await expect(
      new AnthropicProvider().analyzeListings(prompt({ listings: [fixtureListing] })),
    ).rejects.toThrow("Analyzer: no tool_use block in response");
  });

  it("uses code execution + auto tool_choice when knowledge.skillId is set", async () => {
    ...
    const { AnthropicProvider } = await import("./anthropic-provider.js");
    await new AnthropicProvider().analyzeListings(
      prompt({ listings: [fixtureListing], knowledge: { skillId: "skill_01test" } }),
    );
